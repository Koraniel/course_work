...
class AASIST2(nn.Module):
    ...
    def forward(self, data_object, Freq_aug=False, **kwargs):
        ...
        e_S = e_S.transpose(1, 2) + self.pos_S              # [3, 23, 64]

        gat_S = self.GAT_layer_S(e_S)                       # [3, 23, 64]
        out_S = self.pool_S(gat_S)                          # [3, 11, 64]  # (#bs, #node, #dim)

        # temporal GAT (GAT-T)
        e_T, _ = torch.max(torch.abs(e), dim=2)             # [3, 64, 29]  # max along freq
        e_T = e_T.transpose(1, 2)                           # [3, 29, 64]

        gat_T = self.GAT_layer_T(e_T)                       # [3, 29, 64]
        out_T = self.pool_T(gat_T)                          # [3, 20, 64]

        # learnable master node
        master1 = self.master1.expand(x.size(0), -1, -1)    # [3, 1, 64]
        master2 = self.master2.expand(x.size(0), -1, -1)    # [3, 1, 64]
